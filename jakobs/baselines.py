...
class BaselineModel(model.Model):
  ...
  def _loss(self, params, rng_key, feedback, algorithm_index):
    ...
    def do_thing(value):
      ...
      jax.debug.callback(something, value)

    # do_thing(output_preds['s'])
    # print(hint_preds[0].keys())
    # do_thing(hint_preds[0]['m'])
    # do_thing(hint_preds[0]['msk'])
    # do_thing(hint_preds[0]['pred_h'])
    # do_thing(hint_preds[0]['s_h'])
    # do_thing(hint_preds[0]['s')

    # print(hint_preds.keys())
    # print_value('output_preds', output_preds)
    # print_value('hint_preds', hint_preds)
